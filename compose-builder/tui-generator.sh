...
declare -A deviceServiceDesc=(
    ...
    [ds-grove]="Include Grove Device Service (valid only: amd64)"
...
####################################################################
#    amd64 Question Display Function                               #
####################################################################
function displayamd64() {
    local message="Would you like to use amd64 Images to generate the Compose file?"
...
## Step - 1: Start by Asking about whether Images should be pulled for amd64?
amd64_OPT=""
if displayamd64; then
    amd64_OPT="amd64"
fi
...
if finalStep; then
    ...
    $MAKE gen ${amd64_OPT} ${SECURITY_OPT} ${SELECTED_OTHERS[@]} ${SELECTED_DEVSERVICES[@]} ${SELECTED_APPSERVICES[@]} ${SELECTED_BUS[@]}
else
    echo "Generating Compose file and Running the Stack....\n"
    $MAKE run  ${amd64_OPT} ${SECURITY_OPT} ${SELECTED_OTHERS[@]} ${SELECTED_DEVSERVICES[@]} ${SELECTED_APPSERVICES[@]} ${SELECTED_BUS[@]}
fi
