...
- **docker-compose-amd64.yml**
    Contains all the services required to run in secure configuration on `amd64` system.  Includes the Device Virtual & Device REST device services and the UI.
    **Make Commands** 
    
     - Use `make run amd64` and `make down` to start and stop the services using this compose file.
     - Use `make pull amd64 <service(s)>` to pull all or some images for the services in this compose file.
     - Use `make get-token amd64` to generate a Kong access token for remote access of the services running from this compose file.
...
- **docker-compose-with-app-sample-amd64.yml**
    Contains all the services required to run in secure configuration with the Sample application service on `amd64` system .  Includes the Device Virtual, Device REST, UI & App Sample services. Use this version when using the UI to make changes to the configurable pipeline on the Sample application service.

    **Make Commands**

    - Use `make run no-secty app-sample amd64` and `make down` to start and stop the services using this compose file.
...
- **docker-compose-no-secty-amd64.yml**
    Contains just the services needed to run in non-secure configuration on `amd64` system.  Includes the Device Virtual & Device REST device services and the UI.

    **Make Commands**

    - Use `make run no-secty amd64` and `make down` to start and stop the services using this compose file.
    - Use `make pull no-secty amd64 <service(s)>` to pull all or some images for the services in this compose file.
...
- **docker-compose-no-secty-with-app-sample-amd64.yml**
  Contains just the services needed to run in non-secure configuration with the Sample application service on `amd64` system .  Includes the Device Virtual, Device REST, UI & App Sample services. Use this version when using the UI to make changes to the configurable pipeline on the Sample application service.
